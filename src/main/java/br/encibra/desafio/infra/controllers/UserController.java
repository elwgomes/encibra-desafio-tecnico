...
public class UserController {
	...
	@Operation(summary = "Create a new user",
			description = "This endpoint creates a new user and returns the created user details.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "201", description = "User created successfully",
					content = @Content(schema = @Schema(implementation = UserHttpResponse.class))),
			@ApiResponse(responseCode = "400", description = "Invalid input provided"),
			@ApiResponse(responseCode = "500", description = "Internal server error")
	})
	@PostMapping("create")
	public ResponseEntity<UserHttpResponse> create(
			@Parameter(description = "User details for creating a new user", required = true)
			@RequestBody UserHttpRequest request) {
	...
	@Operation(summary = "Get user by ID",
			description = "This endpoint retrieves a user by their ID, with decrypted details.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "User retrieved successfully",
					content = @Content(schema = @Schema(implementation = UserHttpResponseWithList.class))),
