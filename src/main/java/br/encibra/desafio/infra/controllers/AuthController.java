...
public class AuthController {
	...
	@Operation(summary = "Authenticate user", description = "This endpoint authenticates a user by their user ID and generates a JWT token.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Authentication successful", content = @Content(schema = @Schema(implementation = AccessHttpResponse.class))),
			@ApiResponse(responseCode = "404", description = "User not found"),
			@ApiResponse(responseCode = "500", description = "Internal server error")
	})
	@PostMapping("{userId}")
	public ResponseEntity<AccessHttpResponse> authenticate(
			@Parameter(description = "ID of the user to authenticate", required = true) @PathVariable Long userId) {
